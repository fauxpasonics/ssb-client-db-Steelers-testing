...
CREATE VIEW [EmailOutbound].[vw_UpdateList_DEV]
AS
...
WHERE Is_New_Record = 0
	  AND (	   NULLIF(std.First_Name	,'')														<> ISNULL(pdu.FirstName,'') 
			OR NULLIF(std.Last_Name	,'')															<> ISNULL(pdu.LastName,'') 
			OR CASE WHEN std.GENDER IN ('M','F') THEN std.GENDER END								<> ISNULL(pdu.Gender,'') 
			OR COALESCE(std.Birth_Date, NULLIF(TRY_CAST(pdu.DOB AS DATE),'1900-01-01'))				<> ISNULL(CAST(pdu.DOB AS DATE),'1900-01-01') 
			OR NULLIF(cust.DOB_DAY		,'')														<> ISNULL(pdu.DOB_DAY,'') 
			OR NULLIF(cust.DOB_MONTH	,'')														<> ISNULL(pdu.DOB_MONTH,'') 
			OR NULLIF(cust.DOB_YEAR	,'')															<> ISNULL(pdu.DOB_YEAR,'') 
			OR NULLIF(std.Address_City		,'')													<> ISNULL(pdu.City,'') 
			OR CASE WHEN LEN(std.Address_State) = 2 THEN std.Address_State END						<> ISNULL(pdu.State,'') 
			OR CASE WHEN ISNUMERIC(LEFT(std.Address_Zip,5)) = 1 THEN LEFT(std.Address_Zip,5) END	<> ISNULL(pdu.PostalCode,'') 
			OR CASE WHEN LEN(std.Address_Country) = 2 THEN std.Address_Country END					<> ISNULL(pdu.Country,'') 
			OR cust.PREF_SNU																		<> ISNULL(pdu.PREF_SNU,'')  
			OR CAST(cust.SNU_ENROLLED_AT AS DATETIME)												<> ISNULL(CAST(pdu.SNU_ENROLLED_AT AS DATETIME),'1900-01-01') 
			OR cust.SNU_STATUS																		<> ISNULL(pdu.SNU_STATUS,'') 
			OR cust.SNU_TIER																		<> ISNULL(pdu.SNU_TIER,'') 
			OR cust.SNU_TIER_PREVIOUS_SEASON														<> ISNULL(pdu.SNU_TIER_PREVIOUS_SEASON,'') 
			OR cust.SNU_CURRENT_YARDS																<> ISNULL(pdu.SNU_CURRENT_YARDS,'') 
			OR CAST(cust.SNU_LAST_ACTIVITY_DATE AS DATETIME)										<> ISNULL(CAST(pdu.SNU_LAST_ACTIVITY_DATE AS DATETIME),'1900-01-01') 
			OR cust.CURRENT_STH																		<> ISNULL(pdu.CURRENT_STH,'') 
			OR cust.CURRENT_SUITE																	<> ISNULL(pdu.CURRENT_SUITE,'') 
			OR cust.CURRENT_WL																		<> ISNULL(pdu.CURRENT_WL,'') 
			OR cust.TM_SINGLE_BUYER																	<> ISNULL(pdu.TM_SINGLE_BUYER,'') 
			OR cust.TM_CONCERT																		<> ISNULL(pdu.TM_CONCERT,'') 
			OR cust.EVENTS_ATTENDED																	<> ISNULL(pdu.EVENTS_ATTENDED,'') 
			OR cust.CONTESTS_ENTERED																<> ISNULL(pdu.CONTESTS_ENTERED,'') 
			OR cust.IS_WIFI_CUSTOMER																<> ISNULL(pdu.IS_WIFI_CUSTOMER,'') 
			OR cust.IS_MOBILE_APP_CUSTOMER															<> ISNULL(pdu.IS_MOBILE_APP_CUSTOMER,'') 
			OR cust.PRO_SHOP_PURCHASER																<> ISNULL(pdu.PRO_SHOP_PURCHASER,'') 
			OR CAST(cust.PRO_SHOP_LAST_ORDER_DATE AS DATETIME)										<> ISNULL(CAST(pdu.PRO_SHOP_LAST_ORDER_DATE AS DATETIME),'1900-01-01'))
