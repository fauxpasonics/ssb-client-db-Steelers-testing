...
CREATE VIEW [ods].[vw_LoadDimCustomer_Contests] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
        SELECT
            ...
            , 'Contests' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(z.SSID AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
            ...
            , x.SSCreatedDate [SSCreatedDate] --createddate
        ...
        --SELECT COUNT(*)           
        FROM etl.Data_Uploader_Customer_Contests z WITH (NOLOCK) --Change To Source Table
			JOIN (SELECT ssid
						,MIN(Event_Date) SSCreatedDate
