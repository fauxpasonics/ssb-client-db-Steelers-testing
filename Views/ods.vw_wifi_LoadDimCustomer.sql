...
CREATE VIEW [ods].[vw_wifi_LoadDimCustomer] AS (
...
--base set
		SELECT 
			...
			/*Name*/
			, NULL AS [Prefix]
			, CAST(NULLIF(FirstName,'') AS NVARCHAR(100)) AS [FirstName]
			, CAST(NULL AS NVARCHAR(100)) AS [MiddleName]
			, CAST(NULLIF(LastName,'') AS NVARCHAR(100)) AS [LastName]
			...
			, NULL AS [SSCreatedBy]
			, NULL AS [SSUpdatedBy]
			, CAST(SSCreatedDate AS DATETIME) AS [SSCreatedDate]
			, GETDATE() AS [CreatedDate]
			, GETDATE() AS [SSUpdatedDate]
