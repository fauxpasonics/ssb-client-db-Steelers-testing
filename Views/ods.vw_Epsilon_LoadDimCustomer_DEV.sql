...
CREATE VIEW [ods].[vw_Epsilon_LoadDimCustomer_DEV] AS (
...
--		select top 1000 *			
		FROM ods.Epsilon_Profile_Updates (NOLOCK) E
			JOIN   (SELECT WRK.EmailPrimary
					FROM emailoutbound.WorkingSet wrk
						LEFT JOIN (SELECT DISTINCT dc.EmailPrimary
								   FROM dimcustomer dc (NOLOCK)
								  )x ON x.emailPrimary = wrk.emailPrimary
					WHEre x.emailprimary IS NULL
					)m ON m.emailprimary = e.customerkey 
