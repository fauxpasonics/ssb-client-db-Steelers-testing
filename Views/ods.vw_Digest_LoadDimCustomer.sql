...
CREATE VIEW [ods].[vw_Digest_LoadDimCustomer] AS (
...
--base set
		SELECT 
			DB_NAME() AS [SourceDB]
			, (SELECT etl.fnGetClientSetting('Digest-SourceSystem')) AS [SourceSystem]
			...
			, NULL AS [AccountType] 
			, NULL AS [AccountRep] 
			, CAST([Customer Company] AS NVARCHAR (50)) AS [CompanyName] 
			...
			/**ENTITIES**/
			/*Name*/
			, [Address Name Customer Name] AS FullName
			...
			/*AddressPrimary*/
            , CASE 
                WHEN ISNULL([Customer Street],'') = ISNULL([Customer Address2],'') THEN [Customer Street]
                ELSE [Customer Street] + ' ' + [Customer Address2] 
            END AS [AddressPrimaryStreet]
			, [Customer City] AS [AddressPrimaryCity] 
			, [State] AS [AddressPrimaryState] 
			, zip	AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, [Customer Country]	AS [AddressPrimaryCountry] 
			...
			, CAST(NULL AS DATE) AS[ExtAttribute21] --datetime
			, CAST(NULL AS DATE) AS [ExtAttribute22] 
			, CAST(NULL AS DATE) AS[ExtAttribute23] 
			, CAST(NULL AS DATE) AS [ExtAttribute24] 
			, CAST(NULL AS DATE) AS [ExtAttribute25] 
			, CAST(NULL AS DATE) AS [ExtAttribute26] 
			, CAST(NULL AS DATE) AS [ExtAttribute27] 
			, CAST(NULL AS DATE) AS [ExtAttribute28] 
			, CAST(NULL AS DATE) AS [ExtAttribute29] 
			, CAST(NULL AS DATE) AS [ExtAttribute30]  
			...
			, 0 ISDeleted
--		select *				
		FROM src.[Digest_Subscriber_List] (NOLOCK) p
		WHERE p.LoadDate > DATEADD(DAY,-5,GETDATE())
	) a
