...
/*****
	...
	This view contains information regarding CustomerId values in ods.Merch_Order
	that DO NOT have a corresponding record in ods.Merch_Customer.
...
CREATE VIEW [ods].[vw_Merch_GuestOrder_LoadDimCustomer] AS (
...
--base set
		...
		FROM (
			...
			FROM (SELECT MO.CustomerId, MO.BillingAddressId, MO.StoreId,
						ROW_NUMBER() OVER (PARTITION BY MO.CustomerId ORDER BY MO.CreatedOnUtc DESC) RowNum
					FROM ods.Merch_Order MO WITH (NOLOCK)
					LEFT JOIN ods.Merch_Customer MC WITH (NOLOCK)
						ON MC.Id = MO.CustomerId
					WHERE MC.Id IS NULL) MO
