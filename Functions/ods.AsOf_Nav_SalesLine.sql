SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO

/*
-- Get the status of your table 20 minutes ago...
DECLARE @AsOfDate DATETIME = (SELECT [etl].[ConvertToLocalTime](DATEADD(MINUTE,-20,GETDATE())))
SELECT * FROM [ods].[AsOf_Nav_SalesLine] (@AsOfDate)
*/

CREATE FUNCTION [ods].[AsOf_Nav_SalesLine] (@AsOfDate DATETIME)

RETURNS @Results TABLE
(
[DocumentNo] [varchar](20) NULL,
[LineNo] [int] NULL,
[ItemNo] [varchar](20) NULL,
[Description] [varchar](200) NULL,
[Quantity] [int] NULL,
[UnitPrice] [numeric](18, 2) NULL,
[LineDiscountPercent] [numeric](18, 2) NULL,
[LineDiscountAmount] [numeric](18, 2) NULL,
[Amount] [numeric](18, 2) NULL,
[AmountIncludingTax] [numeric](18, 2) NULL,
[DropShipment] [bit] NULL,
[TaxLiable] [bit] NULL,
[LineAmount] [numeric](18, 2) NULL,
[VariantCode] [varchar](20) NULL,
[Category] [varchar](20) NULL,
[SubCategory] [varchar](20) NULL,
[AppliedRuleIDs] [varchar](50) NULL,
[DepartmentDescription] [varchar](50) NULL,
[CategoryDescription] [varchar](50) NULL,
[SubCategoryDescription] [varchar](50) NULL,
[Department] [varchar](50) NULL,
[ETL_CreatedOn] [datetime] NOT NULL,
[ETL_CreatedBy] NVARCHAR(400) NOT NULL,
[ETL_UpdatedOn] [datetime] NOT NULL,
[ETL_UpdatedBy] NVARCHAR(400) NOT NULL
)

AS
BEGIN

DECLARE @EndDate DATETIME = (SELECT [etl].[ConvertToLocalTime](CAST(GETDATE() AS datetime2(0))))
SET @AsOfDate = (SELECT CAST(@AsOfDate AS datetime2(0)))

INSERT INTO @Results
SELECT [DocumentNo],[LineNo],[ItemNo],[Description],[Quantity],[UnitPrice],[LineDiscountPercent],[LineDiscountAmount],[Amount],[AmountIncludingTax],[DropShipment],[TaxLiable],[LineAmount],[VariantCode],[Category],[SubCategory],[AppliedRuleIDs],[DepartmentDescription],[CategoryDescription],[SubCategoryDescription],[Department],[ETL_CreatedOn],[ETL_CreatedBy],[ETL_UpdatedOn],[ETL_UpdatedBy]
FROM
	(
	SELECT [DocumentNo],[LineNo],[ItemNo],[Description],[Quantity],[UnitPrice],[LineDiscountPercent],[LineDiscountAmount],[Amount],[AmountIncludingTax],[DropShipment],[TaxLiable],[LineAmount],[VariantCode],[Category],[SubCategory],[AppliedRuleIDs],[DepartmentDescription],[CategoryDescription],[SubCategoryDescription],[Department],[ETL_CreatedOn],[ETL_CreatedBy],[ETL_UpdatedOn],[ETL_UpdatedBy],@EndDate [RecordEndDate]
	FROM [ods].[Nav_SalesLine] t
	UNION ALL
	SELECT [DocumentNo],[LineNo],[ItemNo],[Description],[Quantity],[UnitPrice],[LineDiscountPercent],[LineDiscountAmount],[Amount],[AmountIncludingTax],[DropShipment],[TaxLiable],[LineAmount],[VariantCode],[Category],[SubCategory],[AppliedRuleIDs],[DepartmentDescription],[CategoryDescription],[SubCategoryDescription],[Department],[ETL_CreatedOn],[ETL_CreatedBy],[ETL_UpdatedOn],[ETL_UpdatedBy],[RecordEndDate]
	FROM [ods].[Snapshot_Nav_SalesLine]
	) a
WHERE
	@AsOfDate BETWEEN [ETL_UpdatedOn] AND [RecordEndDate]
	AND [ETL_CreatedOn] <= @AsOfDate

RETURN

END
GO
