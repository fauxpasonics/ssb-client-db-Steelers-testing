...
UPDATES:		
NOTES:			This PROC loads the data into ods.Yinzcam_Generic, one of four yinzcam segment tables
=============================================================================================================*/


CREATE PROC [etl].[Load_Yinzcam_Generic_DEV]
...
BEGIN TRY
	...
	USING (SELECT *
		   FROM (SELECT *
					   ,RANK() OVER(PARTITION BY YinzID ORDER BY ETL__insert_datetime, NEWID()) rnk
				 FROM #stg
				)x
		   WHERE rnk = 1
		   ) AS MySource
