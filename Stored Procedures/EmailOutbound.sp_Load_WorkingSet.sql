...
CREATE PROC [EmailOutbound].[sp_Load_WorkingSet]
AS 
...
BEGIN TRY
		...
		SELECT dc.EmailPrimary, dc.SourceSystem , MIN(dc.CreatedDate) CreatedDate
		FROM dbo.DimCustomer dc (NOLOCK)	
			JOIN (SELECT SSID
				  FROM EmailOutbound.Data_Uploader_Landing
				  WHERE source = 'contests'
				  GROUP BY SSID
				  HAVING MAX(CAST(Email_Opt_In AS INT)) = 1
				  )OptIns ON Optins.ssid = dc.ssid
			LEFT JOIN #prod prod ON prod.email = dc.EmailPrimary
		WHERE prod.email IS NULL
			  AND dc.SourceSystem = 'contests'
		GROUP BY dc.EmailPrimary, dc.SourceSystem
		...
		UPDATE wrk
		SET wrk.SSB_CRMSYSTEM_CONTACT_ID = COALESCE(EmailGUID.SSB_CRMSYSTEM_CONTACT_ID, ssidGUID.SSB_CRMSYSTEM_CONTACT_ID)
		FROM EmailOutbound.WorkingSet wrk
			LEFT JOIN (SELECT dc.EmailPrimary
					 ...
					 )EmailGUID ON EmailGUID.EmailPrimary = wrk.EmailPrimary
								   AND EmailGUID.rnk = 1
			LEFT JOIN dbo.DimCustomerSSBID ssidGUID ON ssidGUID.SSID = wrk.EmailPrimary	
													   AND ssidGUID.SourceSystem = 'Epsilon'
													   AND wrk.IsNewRecord = 0
