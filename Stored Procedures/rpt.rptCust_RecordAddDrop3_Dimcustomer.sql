...
CREATE PROCEDURE [rpt].[rptCust_RecordAddDrop3_Dimcustomer]

AS 

DECLARE @BatchDate Date = (SELECT MAX(batchDate) FROM dbo.DimCustomer_History_Changes)

IF (SELECT COUNT(*) FROM dbo.DimCustomer_AddDrop_Unpivoted WHERE batchDate = @batchDate) <> 0 
BEGIN
	PRINT 'DimCustomer_AddDrop_Unpivoted has already been populated with the most recent data'
	RETURN
END
...
FROM (SELECT RecordChange, DimCustomerId, BatchDate , SourceSystem, ISNULL(CAST(BatchId AS VARCHAR(100)),'NULL') BatchId	,ISNULL(CAST(ODSRowLastUpdated AS VARCHAR(100)),'NULL') ODSRowLastUpdated ,ISNULL(CAST(SourceDB AS VARCHAR(100)),'NULL') SourceDB	
	...
	WHERE 1=1
		  ...
		  AND recordChange IN ('ADD','DROP','MERGE')
...
INSERT INTO dbo.DimCustomer_AddDrop_Unpivoted

SELECT  unpivoted.BatchDate	
	   ,unpivoted.RecordChange
	   ,unpivoted.dimcustomerID
	   ,unpivoted.SourceSystem
	   ,unpivoted.FieldName
	   ,unpivoted.Value 
FROM #dimcustomerUnpivoted unpivoted
		JOIN dbo.Recordcomparefields rcf ON rcf.fieldName = unpivoted.FieldName
WHERE rcf.isToBeCompared = 1
