...
CREATE PROCEDURE [rpt].[rptCust_RecordAddDrop4_Dimcustomer]
...
IF (SELECT COUNT(*) FROM dimcustomer_changes_unpivot WHERE batchDate = @batchDate) = 0
BEGIN 
	PRINT 'dimcustomer_changes_unpivot has already been populated with the most recent data'
	RETURN
END
...
FROM (SELECT BatchDate, RecordChange, DimCustomerId, SourceSystem, SnapshotDate	,ISNULL(CAST(BatchId AS VARCHAR(100)),'NULL') BatchId	,ISNULL(CAST(ODSRowLastUpdated AS VARCHAR(100)),'NULL') ODSRowLastUpdated ,ISNULL(CAST(SourceDB AS VARCHAR(100)),'NULL') SourceDB	
...
INSERT INTO dimcustomer_Changes_Unpivot
...
SELECT BatchDate	
	  ...
	  ,PriorDayValue	
	  ,CurrentDayValue
FROM ( SELECT PriorDay.BatchDate
			, PriorDay.RecordChange
			, PriorDay.dimcustomerID
			, PriorDay.fieldName
