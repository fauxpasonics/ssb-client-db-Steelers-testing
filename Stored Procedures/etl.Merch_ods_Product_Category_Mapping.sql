...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[Merch_ods_Product_Category_Mapping]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.Merch_Product_Category_Mapping),'0');	
...
,  LastTimeStamp, Id, DisplayOrder, ProductId, CategoryId, IsFeaturedProduct
INTO #SrcData
FROM (
	SELECT  LastTimeStamp, Id, DisplayOrder, ProductId, CategoryId, IsFeaturedProduct
	, ROW_NUMBER() OVER(PARTITION BY Id ORDER BY ETL_ID) RowRank
	FROM src.Merch_Product_Category_Mapping
) a
WHERE RowRank = 1
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(CONVERT(varchar(10),CategoryId)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),DisplayOrder)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),Id)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),IsFeaturedProduct)),'DBNULL_BIT') + ISNULL(RTRIM(LastTimeStamp),'DBNULL_TEXT') + ISNULL(RTRIM(CONVERT(varchar(10),ProductId)),'DBNULL_INT'))
...
MERGE ods.Merch_Product_Category_Mapping AS myTarget
...
THEN UPDATE SET
     ...
     , myTarget.[IsFeaturedProduct] = mySource.[IsFeaturedProduct]
...
VALUES
     ...
     ,mySource.[ProductId]
     ,mySource.[CategoryId]
     ,mySource.[IsFeaturedProduct]
     )
;

EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Merge Load', 'Merge Statement Execution', 'Complete', @ExecutionId

DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_Product_Category_Mapping WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_Product_Category_Mapping WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
