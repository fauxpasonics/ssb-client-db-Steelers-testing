...
CREATE PROCEDURE [rpt].[FactTickettoMerch_20160217] 
...
--DROP TABLE #CContact
--DROP TABLE #CContact_CLub
--DROP TABLE #CContact_Suite
--DROP TABLE #CContact_GA
--DROP TABLE #CContact_Merch
...
(Sum(isnull(Club$,0)))*1.0/nullif(Sum(isnull(Club#,0)),0) as AvgClubTicket,
Sum(isnull(GA#,0)) as TotalGATickets,
Sum(isnull(GA$,0)) as TotalGASpend,
(Sum(isnull(GA$,0)))*1.0/nullif(Sum(isnull(GA#,0)),0) as AvgGATicket,
Sum(isnull(Suite#,0)) as TotalSuiteTickets,
Sum(isnull(Suite$,0)) as TotalSuiteSpend,
(Sum(isnull(Suite$,0)))*1.0/nullif(Sum(isnull(Suite#,0)),0) as AvgSuiteTicket,
Sum(isnull(Merch#,0)) as TotalMerchCount,
Sum(isnull(Merch$,0)) as TotalMerchSpend,
(Sum(isnull(Merch$,0)))*1.0/nullif(Sum(isnull(Merch#,0)),0) as AvgMerchSpend,
...
(Sum(case when isnull(Club#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0 /nullif(Sum(case when isnull(Club#,0) > 0 then 1 else 0 end),0) as AvgClubMerch,
...
(Sum(case when isnull(GA#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0/nullif(Sum(case when isnull(GA#,0) > 0 then 1 else 0 end),0) as AvgGaMerch,
...
(Sum(case when isnull(Suite#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0/nullif(Sum(case when isnull(Suite#,0) > 0 then 1 else 0 end),0) as AvgSuiteMerch,
(Sum(case when isnull(Club#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0/nullif(Sum(isnull(Club#,0)),0) as AvgMerchPerTicketClub,
(Sum(case when isnull(GA#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0/nullif(Sum(isnull(GA#,0)),0) as AvgMerchPerTicketGA,
(Sum(case when isnull(Suite#,0) > 0 then isnull(Merch$,0) else 0 end))*1.0/nullif(Sum(isnull(Suite#,0)),0) as AvgMerchPerTicketSuite
FROM #CContact
