...
CREATE PROCEDURE [etl].[TM_LoadFactTicketSales_History_temp]
(
	@BatchId INT = 0,
...
SELECT 
	...
	--SELECT COUNT(*)
	FROM #WorkingSet tkt
	LEFT OUTER JOIN dbo.DimItem (nolock) dItem ON CASE WHEN tkt.plan_event_id = 0 THEN tkt.event_id ELSE tkt.plan_event_id END = dItem.SSID_event_id AND dItem.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dItem.DimItemId > 0
	...
	ALTER INDEX [IDX_LoadKey] ON dbo.FactTicketSalesHistory REBUILD
	ALTER INDEX [IDX_DeltaHashKey] ON dbo.FactTicketSalesHistory REBUILD
