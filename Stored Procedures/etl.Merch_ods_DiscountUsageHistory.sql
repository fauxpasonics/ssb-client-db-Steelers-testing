...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[Merch_ods_DiscountUsageHistory]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.Merch_DiscountUsageHistory),'0');	
...
,  Id, DiscountId, OrderId, CreatedOnUtc, LastTimeStamp
INTO #SrcData
FROM (
	SELECT  Id, DiscountId, OrderId, CreatedOnUtc, LastTimeStamp
	, ROW_NUMBER() OVER(PARTITION BY Id ORDER BY ETL_ID) RowRank
	FROM src.Merch_DiscountUsageHistory
) a
WHERE RowRank = 1
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(CONVERT(varchar(25),CreatedOnUtc)),'DBNULL_DATETIME') + ISNULL(RTRIM(CONVERT(varchar(10),DiscountId)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),Id)),'DBNULL_INT') + ISNULL(RTRIM(LastTimeStamp),'DBNULL_TEXT') + ISNULL(RTRIM(CONVERT(varchar(10),OrderId)),'DBNULL_INT'))
...
MERGE ods.Merch_DiscountUsageHistory AS myTarget
...
THEN UPDATE SET
     ...
     , myTarget.[DiscountId] = mySource.[DiscountId]
...
DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_DiscountUsageHistory WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_DiscountUsageHistory WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
