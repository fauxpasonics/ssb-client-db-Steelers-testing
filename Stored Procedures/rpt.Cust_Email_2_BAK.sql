...
CREATE PROC [rpt].[Cust_Email_2_BAK]
AS
...
SELECT EmailPrimary, CustomerStatus,DimCustomerId, SSID
INTO #custcust
FROM dimcustomer --WHERE sourcesystem = 'epsilon'

SELECT DISTINCT SSB_CRMSYSTEM_CONTACT_ID, dc.SSID, dc.DimCustomerId,dc.CustomerStatus, dc.EmailPrimary
INTO #tempcust
FROM 
dbo.DimCustomerSSBID a (NOLOCK) 
JOIN #custcust dc (NOLOCK)
 ON dc.DimCustomerId = a.DimCustomerId
