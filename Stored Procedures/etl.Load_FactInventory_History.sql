...
CREATE PROCEDURE [etl].[Load_FactInventory_History]
...
*/
	...
	INSERT INTO dbo.FactInventoryHistory (
	...
	LEFT OUTER JOIN dbo.FactInventoryHistory fi ON fi.DimEventId = de.DimEventId AND fi.DimSeatId = dst.DimSeatId
...
*/
	...
	FROM dbo.FactInventoryHistory f
	...
	inner join dbo.FactInventoryHistory f on sas.DimEventId = f.DimEventId and sas.DimSeatId = f.DimSeatId
	WHERE (
...
*/
	...
	inner join dbo.FactInventoryHistory f on shs.DimEventId = f.DimEventId and shs.DimSeatId = f.DimSeatId
	WHERE (
	f.IsHeld = 0
...
SELECT 
	...
	FROM [rpt].[vw_FactTicketSales_All] (NOLOCK) f
...
*/
	...
	FROM dbo.FactInventoryHistory (NOLOCK) f
	...
	update dbo.FactInventoryHistory
	SET IsAvailable = 0 
