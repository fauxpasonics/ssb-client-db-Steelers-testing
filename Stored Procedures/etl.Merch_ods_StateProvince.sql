...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[Merch_ods_StateProvince]
(
	@BatchId INT = 0,
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.Merch_StateProvince),'0');	
...
,  Id, Name, DisplayOrder, LastTimeStamp, CountryId, Abbreviation, Published
INTO #SrcData
FROM (
	SELECT  Id, Name, DisplayOrder, LastTimeStamp, CountryId, Abbreviation, Published
	, ROW_NUMBER() OVER(PARTITION BY Id ORDER BY ETL_ID) RowRank
	FROM src.Merch_StateProvince
) a
WHERE RowRank = 1
...
UPDATE #SrcData
SET ETL_DeltaHashKey = HASHBYTES('sha2_256', ISNULL(RTRIM(Abbreviation),'DBNULL_TEXT') + ISNULL(RTRIM(CONVERT(varchar(10),CountryId)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),DisplayOrder)),'DBNULL_INT') + ISNULL(RTRIM(CONVERT(varchar(10),Id)),'DBNULL_INT') + ISNULL(RTRIM(LastTimeStamp),'DBNULL_TEXT') + ISNULL(RTRIM(Name),'DBNULL_TEXT') + ISNULL(RTRIM(CONVERT(varchar(10),Published)),'DBNULL_BIT'))
...
MERGE ods.Merch_StateProvince AS myTarget
...
THEN UPDATE SET
     ...
     , myTarget.[Abbreviation] = mySource.[Abbreviation]
...
DECLARE @MergeInsertRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_StateProvince WHERE ETL_CreatedDate >= @RunTime AND ETL_UpdatedDate = ETL_CreatedDate),'0');	
DECLARE @MergeUpdateRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM ods.Merch_StateProvince WHERE ETL_UpdatedDate >= @RunTime AND ETL_UpdatedDate > ETL_CreatedDate),'0');	
